...
#include "AnimGraphNode_MagicTwoBoneIK.h"
...
#define LOCTEXT_NAMESPACE "AnimGraphNode_MagicTwoBoneIK"
...
TSharedPtr<FTwoBoneIKDelegate> UAnimGraphNode_MagicTwoBoneIK::TwoBoneIKDelegate = NULL;

/////////////////////////////////////////////////////
// UAnimGraphNode_MagicTwoBoneIK


UAnimGraphNode_MagicTwoBoneIK::UAnimGraphNode_MagicTwoBoneIK(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

FText UAnimGraphNode_MagicTwoBoneIK::GetControllerDescription() const
{
	return LOCTEXT("MagicTwoBoneIK", "Magic Two Bone IK");
}

FText UAnimGraphNode_MagicTwoBoneIK::GetTooltipText() const
{
	return LOCTEXT("AnimGraphNode_MagicTwoBoneIK_Tooltip", "The Two Bone IK control applies an inverse kinematic (IK) solver to a 3-joint chain, such as the limbs of a character. Can use in AnimBlueprint Template.");
}

FText UAnimGraphNode_MagicTwoBoneIK::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
...
void UAnimGraphNode_MagicTwoBoneIK::CopyNodeDataToPreviewNode(FAnimNode_Base* InPreviewNode)
{
	FAnimNode_MagicTwoBoneIK* TwoBoneIK = static_cast<FAnimNode_MagicTwoBoneIK*>(InPreviewNode);
...
void UAnimGraphNode_MagicTwoBoneIK::CopyPinDefaultsToNodeData(UEdGraphPin* InPin)
{
	if (InPin->GetName() == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, EffectorLocation))
	{
		GetDefaultValue(GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, EffectorLocation), Node.EffectorLocation);
	}
	else if (InPin->GetName() == GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, JointTargetLocation))
	{
		GetDefaultValue(GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, JointTargetLocation), Node.JointTargetLocation);
	}
}

void UAnimGraphNode_MagicTwoBoneIK::CustomizeDetails(class IDetailLayoutBuilder& DetailBuilder)
{
	...
	const FString EffectorLocationSpace = FString::Printf(TEXT("Node.%s"), GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, EffectorLocationSpace));
	const FString JointTargetLocationSpace = FString::Printf(TEXT("Node.%s"), GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, JointTargetLocationSpace));
	...
	const FString TakeRotationPropName = FString::Printf(TEXT("Node.%s"), GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, bTakeRotationFromEffectorSpace));
	const FString EffectorTargetName = FString::Printf(TEXT("Node.%s"), GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, EffectorTarget));
	const FString EffectorLocationPropName = FString::Printf(TEXT("Node.%s"), GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, EffectorLocation));
	...
	const FString JointTargetName = FString::Printf(TEXT("Node.%s"), GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, JointTarget));
	const FString JointTargetLocation = FString::Printf(TEXT("Node.%s"), GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_MagicTwoBoneIK, JointTargetLocation));
...
FEditorModeID UAnimGraphNode_MagicTwoBoneIK::GetEditorMode() const
{
	return AnimNodeEditModes::TwoBoneIK;
}

void UAnimGraphNode_MagicTwoBoneIK::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
	...
	if (CustomAnimVersion < FAnimationCustomVersion::ConvertIKToSupportBoneSocketTarget)
	{
		...
		{
			Node.EffectorTarget = FMagicBoneSocketTarget(Node.EffectorSpaceBoneName_DEPRECATED);
		}

		if (Node.JointTargetSpaceBoneName_DEPRECATED != NAME_None)
		{
			Node.JointTarget = FMagicBoneSocketTarget(Node.JointTargetSpaceBoneName_DEPRECATED);
		}
	}
}

void UAnimGraphNode_MagicTwoBoneIK::Draw(FPrimitiveDrawInterface* PDI, USkeletalMeshComponent* SkelMeshComp) const
{
	if (bEnableDebugDraw && SkelMeshComp)
	{
		if (FAnimNode_MagicTwoBoneIK* ActiveNode = GetActiveInstanceNode<FAnimNode_MagicTwoBoneIK>(SkelMeshComp->GetAnimInstance()))
		{
